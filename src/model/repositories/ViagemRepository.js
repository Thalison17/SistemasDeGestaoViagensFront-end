...
export default class ViagemRepository {
      ...
      return response.data.value.map(
        (viagem) =>
          new Viagem(
            ...
            viagem.dataPartida,
            viagem.dataRetorno,
            viagem.cliente, // Assuming cliente is an object with necessary properties
            viagem.destino, // Assuming destino is an object with necessary properties
          ),
      )
    } catch (error) {
  ...
  async updateViagem(id, form) {
    try {
      const baseRoute = this.createBaseRoute()
      form.viagemId = id // Assuming viagemId is used for updates
