...
import { useClienteStore } from 'src/controller/store/ClienteStore'
import { useDestinoStore } from 'src/controller/store/DestinoStore'
...
export default {
  setup() {
    ...
    const filteredReservas = computed(() => {
      ...
      if (searchText.value) {
        ...
        reservas = reservas.filter(
          (reserva) =>
            reserva.cliente?.nome?.toLowerCase().includes(searchLower)
        )
      }
    ...
    const showDetails = async (reserva) => {
      try {
        const cliente = await clienteStore.fetchById(reserva.clienteId)
        const destino = await destinoStore.fetchById(reserva.viagem.destinoId)
        ...
        detailsForm.value = {
          ...
          status: reserva.viagem.status || 'N/A'
        }
    ...
    const updateReserva = async () => {
      try {
        loading.value = true
        const updateData = {
          reservaId: detailsForm.value.reservaId,
        ...
        await reservaStore.updateItem(updateData.reservaId, updateData)
        ...
        await fetchReservas() // Recarrega a lista
      } catch (error) {
